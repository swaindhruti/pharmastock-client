...
export default function Home() {
  ...
  return (
    <div
      ref={container}
      className="min-h-screen bg-bg-main text-text-main font-faktum selection:bg-brand/20 selection:text-brand flex flex-col relative overflow-hidden"
    >
      <Navbar />
      ...
      <main className="w-full mx-auto px-6 md:px-16 pt-32 pb-24 flex flex-col items-start justify-end text-left relative z-10 grow min-h-[80vh]">
        ...
        <div className="relative z-10 max-w-5xl flex flex-col items-start w-full mt-auto">
          {/* Massive Headline (Desktop - Split Text) */}
          <h1 className="hidden md:block text-5xl sm:text-6xl md:text-7xl lg:text-[6.5rem] font-medium tracking-tight mb-8 text-text-main leading-[1.05]">
            {splitText("Intelligent Search for")}{" "}
            <br />{" "}
            <span className="text-text-main">
              {splitText("Indian Healthcare")}
            </span>
          </h1>

          {/* Massive Headline (Mobile - Standard Fade) */}
          <h1 className="gsap-hero-element md:hidden text-5xl sm:text-6xl font-medium tracking-tight mb-8 text-text-main leading-[1.05]">
            Intelligent Search for <br /> <span className="text-text-main">Indian Healthcare</span>
          </h1>

          {/* Subtext */}
          <p className="gsap-hero-element text-lg md:text-2xl text-text-muted max-w-2xl font-medium leading-relaxed mb-12">
            With our state-of-the-art zero latency search, you can instantly
            lookup medicines, map generic substitutes, and view side effects in
            seconds.
          </p>
          ...
          <div className="gsap-hero-element flex flex-col sm:flex-row gap-6 mt-4">
            <Link
              href="/search"
              className="group flex items-center justify-center gap-3 bg-text-main text-white px-8 py-4 rounded-full font-bold tracking-wide transition-all shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-none hover:translate-x-[4px] hover:translate-y-[4px]"
            >
            ...
            <Link
              href="/about"
              className="group flex items-center justify-center gap-3 bg-transparent border-2 border-text-main text-text-main px-8 py-4 rounded-full font-bold tracking-wide transition-all shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-none hover:translate-x-[4px] hover:translate-y-[4px] hover:bg-surface-hover"
            >
