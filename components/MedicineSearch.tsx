...
export default function MedicineSearch() {
  ...
  return (
    <div
      ref={containerRef}
      className="w-full h-full flex flex-col gap-6 lg:gap-12 font-faktum relative z-10 pt-4 md:pt-0 max-w-4xl mx-auto"
    >
      {/* Header and Search Bar (Top) */}
      <div className="w-full flex flex-col justify-start shrink-0">
        <div className="mb-8">
          <h1 className="gsap-search-left text-3xl sm:text-4xl md:text-6xl font-medium tracking-tight md:mb-4 text-text-main leading-[1.05]">
            Indian Medical Database
          </h1>
          <p className="gsap-search-left hidden md:block text-base sm:text-lg text-text-muted font-medium leading-relaxed">
            Instantly lookup medicines, map generic substitutes, and view
            side-effects using our zero-latency search index.
          </p>
        </div>
        ...
        <div className="gsap-search-left relative z-10 w-full group">
          <div className="relative flex items-center bg-transparent border-b-4 border-text-main focus-within:border-text-muted transition-colors">
            <input
              ...
              className="w-full md:py-6 bg-transparent text-text-main focus:outline-none placeholder:text-text-muted/40 text-2xl sm:text-3xl md:text-4xl font-black tracking-tight"
              value={searchTerm}
            ...
            <div className="absolute right-12">
              {loading && (
                <div className="w-8 h-8 border-4 border-text-muted/20 border-t-text-main rounded-full animate-spin" />
              )}
            </div>
            {/* Decorative Icon */}
      ...
      <div className="w-full h-full grow overflow-y-auto pb-32 lg:pt-12 pr-4 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] scrollbar-none relative">
        {/* Initial Empty State */}
        ...
        {loading && results.length === 0 && searchTerm.length >= 3 && (
          <ul className="space-y-6">
            {[...Array(5)].map((_, i) => (
              <li
                key={i}
                className="p-8 rounded-3xl border-2 border-text-main bg-transparent animate-pulse flex flex-col gap-4"
              >
        ...
        {searchTerm.length >= 3 && !loading && results.length === 0 && (
          <div className="p-12 rounded-3xl bg-transparent border-2 border-text-main text-center flex flex-col items-center justify-center shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
            <p className="text-text-main font-black text-3xl mb-4">
              No results for &ldquo;{searchTerm}&rdquo;
            </p>
            <p className="text-xl font-medium text-text-muted">
              Try checking the spelling or searching by therapeutic class.
            </p>
          </div>
        )}
        ...
        {!loading && results.length > 0 && (
          <div className="pb-20">
            <p className="text-text-muted font-bold tracking-widest text-sm uppercase mb-6 px-2">
        ...
        createPortal(
          <div className="fixed inset-0 z-[9999] bg-bg-main/90 backdrop-blur-md flex items-center justify-center p-4 transition-opacity duration-300 opacity-100 animate-[fadeIn_0.3s_ease-out]">
            <div className="bg-white border-4 border-text-main rounded-3xl p-6 md:p-12 max-w-lg w-full shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] transform transition-all animate-[slideUp_0.4s_ease-out]">
              <h2 className="text-3xl md:text-4xl font-black mb-3 md:mb-4 text-text-main tracking-tight">
                One quick question...
              </h2>
              <p className="text-sm md:text-base text-text-muted font-medium mb-6 md:mb-8">
                We&apos;re currently in beta. To help us improve
                SmartDrugFinder, please let us know a bit about yourself before
                viewing the details.
              </p>
              <form
              ...
              >
                <div className="flex flex-col gap-2">
                  <label className="font-bold text-text-main uppercase tracking-widest text-[10px] md:text-xs">
                    I am a
                  </label>
                  <select
